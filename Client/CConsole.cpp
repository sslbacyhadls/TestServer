...
//Считывание комманд и аргументов, запуск функций по заданным параметрам 
...
bool Console::launch(char** args) {
	...
	else if (command == "Clear" || command == "cls" || command == "clear")
	{
		...
		if (arg1 == "0") {
			client.Cconnect((char*)"127.0.0.1", (char*)"50000");
		}
		else if (arg2 == "0") {
			client.Cconnect(args[1], (char*)"50000");
		}
		else if (arg1 != "0", arg2 != "0") {
			client.Cconnect(args[1], args[2]);
		}
	}
	else if (command == "send" || command == "Send") 
	{
		std::cout << "Sending: " << "Testing"<< std::endl;
		client.sendData((char*)"Testing");
	};
}